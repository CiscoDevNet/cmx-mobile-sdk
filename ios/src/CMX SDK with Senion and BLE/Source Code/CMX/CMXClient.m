...
@property (strong, nonatomic) CLBeaconRegion *scanningBeaconRegion;
@property (strong, nonatomic) CLLocationManager *locationManager;
@property BOOL beaconDetected;

@property (strong,nonatomic) SLIndoorLocationManager *slLocationManager;
@property (strong,nonatomic) BuildingInfo *buildingInfo;
@property bool doRotate;
@property bool doCentering;
@property double lastHeading;
@property CGPoint lastCenterPos;
    //	SLCoordinate3D* refLoc;
@property NSInteger currentFloorNrUser;
@property SLPoint3D *currentPixPos;
@property double floorWidth,floorLength;
@property double bleLastUpdateTime;
@property double uncertaintyRadius;

@property CMXSenionLocation *currentLocation;
...
-(id)init {
    ...
    if (self) {
        ...
        self.scanningBeaconRegion = [[CLBeaconRegion alloc] initWithProximityUUID:uuid
                                                                 identifier:@"com.appcoda.testregion"];
        
        // Tell location manager to start monitoring for the beacon region
        CMXLog(@"Starting Location Manager");
        [self.locationManager startMonitoringForRegion:self.scanningBeaconRegion];
        
        // Check if beacon monitoring is available for this device
        if (![CLLocationManager isMonitoringAvailableForClass:[CLBeaconRegion class]]) {
            CMXLog(@"Monitoring not available");
        ...
        //Initialize the Senion Lab Indoor Framework
        
        //init the Senion Lab SDK here
        NSString *mapId = @"34e56906-098c-4287-a551-96f944670e5d"; // Obtained from SenionLab
        NSString *customerId = @"479974f1-3e8b-4410-add3-8629eb81bb6f" ; // Obtained from SenionLab
        self.slLocationManager = [[SLIndoorLocationManager alloc] initWithMapId:mapId andCustomerId:customerId];
        self.buildingInfo = [[BuildingInfo alloc] initWithBitmapJsonFilename:@"ciscoOffice14_iOS_bi"];
        self.slLocationManager.delegate = self;
...
-(void) initWithRegistrationCompletion:(void (^)())completion
     ...
     __block NSArray* venueInfo;
    [self registerForRemoteNotifications];
    [self internalLoadVenues:[self getVenuesInfosURL] start:nil
...
- (void)application:(UIApplication *)app didRegisterForRemoteNotificationsWithDeviceToken:(NSData *)devToken {
    ...
    NSDictionary* userInfo = [NSDictionary dictionaryWithObjectsAndKeys:devToken, CMXRemoteNotificationsDeviceTokenKey, nil];
...
- (void)locationManager:(CLLocationManager*)manager didEnterRegion:(CLRegion *)region
{
    ...
    CMXLog(@"Entered a beacon region");
    UILocalNotification *notification = [[UILocalNotification alloc] init];
    notification.alertBody = [NSString stringWithFormat:@"You entered region %@", region.identifier];
    [[UIApplication sharedApplication] presentLocalNotificationNow:notification];
    
    [self.locationManager startRangingBeaconsInRegion:self.scanningBeaconRegion];
...
{
    // Exited the region
    CMXLog(@"Exited beacon region");
    [self.locationManager stopRangingBeaconsInRegion:self.scanningBeaconRegion];
    self.beaconDetected = NO;
}
////////////////////////////////////////////////////////////////////////////////////////////////////
              ...
              inRegion:(CLBeaconRegion*)region
{
    ...
    if ((foundBeacon.proximity == 1)|| (foundBeacon.proximity == 2 )){
        CMXLog(@"Beacon Found");
        self.beaconDetected =  YES;
    }else{
        self.beaconDetected = NO;
    }
}
...
{
    self.currentFloorNrUser = location.floorNr;
	//self.currentPixPos = [self.buildingInfo longLat2PixelPoint:location];
    //self.bleLastUpdateTime = [[NSDate date] timeIntervalSince1970];
    //self.uncertaintyRadius = radius;
    
    //get the seconds part from the timestamp
    
    
    self.currentLocation = [[CMXSenionLocation alloc] initSenionLocationWithLocation:[self.buildingInfo longLat2PixelPoint:location] andTimeStamp:[[NSDate date] timeIntervalSince1970] andUncertaintyRadius:radius];
    
    
    CMXLog(@"Current Location is %@",self.currentLocation);
    
    
        //FloorInfo* f = [self.buildingInfo getFloorInfo:self.currentFloorNrUser];
        //double pixRadius = radius * f.pixelsPerMeter;
        //self.clientLocation.mapCoordinate.x = (pixPoint.x - 306.890382);
        //self.clientLocation.mapCoordinate.y = (pixPoint.y - 178.534665);
    //CMXLog(@"Senion Relative Location in meters is %@",self.currentPixPos);
}
...
{
	double pixHeading = [self.buildingInfo heading2PixelHeading:heading andFloorNr:self.currentFloorNrUser];
    self.lastHeading = pixHeading;
    //CMXLog(@"lastheading is %f",self.lastHeading);
...
- (void) didFailInternetConnectionWithError:(NSError *)error {
    //UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Error" message:@"Please enable the internet connection!" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
    //[alert show];
}
...
-(void)loadUserLocationWithCompletion:(void (^) (CMXClientLocation *clientLocation))completion
    ...
    [_httpsConnectionManager getPath:[self getClientLocationURL] parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        CMXLog(@"Client Relative CMX Location is %@",clientLocation.mapCoordinate);
        
        if (self.currentLocation){
        
                //this call is needed only if the value is in pixel ratio
                [self loadFloor:clientLocation.floorId
                        ofVenue:clientLocation.venueId
                          start:nil
                     completion:^(CMXFloor *floor) {
                         self.floorWidth = floor.dimension.width;
                         self.floorLength = floor.dimension.length;
                                
                     } failure:^(NSError *error) {
                 
                     }];
       
            CMXLog(@"CMX Location Update Time is %@",self.clientLocation.lastLocationUpdateTime);
            CMXLog(@"Senion Location Update Time is %f",self.bleLastUpdateTime);
        
            CMXSenionLocation *senionLocation = self.currentLocation;
        
            //this is if the value is in pixel ratio
            clientLocation.mapCoordinate.x = (senionLocation.x * self.floorWidth) ;
            clientLocation.mapCoordinate.y = (senionLocation.y * self.floorLength) ;
            clientLocation.mapCoordinate.unit = FEET;
            //This is if the value is in pixels
            //clientLocation.mapCoordinate.x = self.currentPixPos.x;
            //clientLocation.mapCoordinate.y = self.currentPixPos.y;
            //clientLocation.mapCoordinate.unit = PIXEL;
      
            self.clientLocation = clientLocation;
            CMXLog(@"Client Relative Senion Location is %@",self.clientLocation.mapCoordinate);
            CMXLog(@"Uncertainty in meters is %f",senionLocation.uncertaintyRadius);
            CMXLog(@"Client Geo Location is %@",self.clientLocation.geoCoordinate);
        }
        
        /*Check if an iBeacon has been detected with proximity 1 or 2
         If it is detected check if the locations are the same or not
         If beacon is detected  with close proximity overwrite with the Beacon's Location*/
        if (self.beaconDetected == YES) {
            self.clientLocation.mapCoordinate.x = 100;
            self.clientLocation.mapCoordinate.y = 100;
        }
        if(completion) {
...
-(void)internalLoadVenues:(NSString*)url
    ...
    [_httpsConnectionManager getPath:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        //loading all venues - respnseobject is an array of venues
