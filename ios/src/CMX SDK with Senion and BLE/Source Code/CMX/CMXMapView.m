...
-(void) setupWithMapImage:(UIImage *)mapImage mapDimension:(CMXDimension*)dimension {
    ...
    if (mapImage != nil) {
        ...
        CMXLog(@"Image size width,height is %f %f",imageSize.width,imageSize.height);
        ...
        CMXLog(@"Root Image size is %f %f",self.rootImageSize.width,self.rootImageSize.height);
        self.contentSize = imageSize;
        CMXLog(@"Content size is %f %f",self.contentSize.width,self.contentSize.height);
...
-(void)showUserLocation:(CMXClientLocation*)userLocation withOrientation:(float)orientation image:(UIImage*)image {
    //CMXLog(@"We are in showUserLocation");
    if(!userLocation || userLocation == nil) {
        // No location, hide the view
        //CMXLog(@"No user location");
        [_userImageView removeFromSuperview];
        _userImageView.hidden = YES;
    }
    else {
        //CMXLog(@"User location found area");
...
-(CGPoint) convertCoordinate:(CMXMapCoordinate*)coordinate {
    if (coordinate.unit == PIXEL) {
        return CGPointMake(coordinate.x, coordinate.y);
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
-(CGPoint) convertX:(double)x y:(double)y {
    CMXLog(@"x,y before conversion is %f %f",x,y);
    CMXLog(@"Rootimage size width,height is %f %f",self.rootImageSize.width,self.rootImageSize.height);
    CMXLog(@"Map dimension width,height is %f %f",self.mapDimension.width,self.mapDimension.length);
    CGPoint p = CGPointMake((x  * self.rootImageSize.width) / _mapDimension.width,
                            (y  * self.rootImageSize.height) / _mapDimension.length);
    CMXLog(@"Scaled x,y is %f %f ",p.x,p.y);
